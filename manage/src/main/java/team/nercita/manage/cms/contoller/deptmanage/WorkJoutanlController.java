...
public class WorkJoutanlController {
	...
	public ModelAndView detail(@ModelAttribute(value="userId") String userId, @ModelAttribute(value="startTime") String startTime,@ModelAttribute(value="endTime") String endTime){
		...
		view.addObject("workJoutanlLists", workJoutanlService.doJoinTransFindWorkJoutanl(userId, beginDate, endDate));
	...
	@RequestMapping("/toadds/{userid}")
	@ResponseBody
	public JSONArray toadds(@PathVariable String userid){
		List<WorkJoutanl> lists = workJoutanlService.doJoinTransFindWorkJoutanlByuserid(userid);
		JSONArray json = new JSONArray();
        for(WorkJoutanl pLog : lists){
            JSONObject jo = new JSONObject();
            jo.put("worklog", pLog.getWorkContent());//工作日志
            jo.put("joutanlDate",sdf.format(pLog.getJoutanlDate()));//日志时间
           
            User user  = userService.doJoinTransFindUserById(userid);
            jo.put("username", user.getName());//日志人
