...
public class ProductStoreServiceImpl extends BaseService implements ProductStoreService {
	...
	@Override
	public void doTransUpdateProductStore(ProductStore productStore) {
		List<ProductStoreItem> itemList = productStore.getItemList();//新添加的产品规格型号
		
		String sql = "select p from ProductStoreItem p where p.productStore.id = :ID";//原有的产品规格型号
		Map<String, Object> queryMap = new HashMap<String, Object>();
		queryMap.put("ID", productStore.getId());
		
		List<ProductStoreItem> oldList =  (List<ProductStoreItem>)baseDao.findObjectList(sql, queryMap);
		if(oldList != null && !oldList.isEmpty()) {
			for (ProductStoreItem old : oldList) {
				baseDao.delete(old);
			}
		}
		for (ProductStoreItem productStoreItem : itemList) {
