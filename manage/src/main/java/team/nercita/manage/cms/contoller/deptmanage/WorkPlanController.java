...
public class WorkPlanController {
	...
	@ResponseBody
	@RequestMapping("/yq/{id}")
	public List<AlertWinVo> yq(@PathVariable String id){
		List<AlertWinVo> voList = new ArrayList();
		
		List<PlanLogYq> yqList = workPlanService.doJoinTransQeuryYqById(id);
		
		DateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		
		if(yqList != null && !yqList.isEmpty()) {
			for (PlanLogYq yqLog : yqList) {
				AlertWinVo vo = new AlertWinVo();
				vo.setStr1(df.format(yqLog.getOldEndTime()));
				vo.setStr2(df.format(yqLog.getNewEndTime()));
				vo.setStr3(yqLog.getYy());
