...
public class ProjectServiceImpl extends BaseService implements ProjectService{
	...
	public Map<String, Object> doJoinTransFindProjectList(Integer goPage, Map<String, Object> paramMap) {
		...
		String sql = "select a from Project a left join fetch a.incomeList left join fetch a.user left join fetch a.userGroup where 1=1 and a.status = 0";
	...
	public Project doJoinTransFindProject(String id) {
		...
		sql = "select f from FundsIncome f where f.project.id = :PID";
		
		project.setIncomeList((List<FundsIncome>) baseDao.findObjectList(sql, paramMap));
		return project;
	}
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<Project> doJoinTransQueryProjects() {
		String sql = "select p from Project p where p.status = 0 and p.type = 1";
		return (List<Project>) baseDao.findObjectList(sql, null);
	}
	...
	public List<FundsDetail> doJoinTransQueryDetail(String id) {
		String sql = "select f from FundsDetail f left join fetch f.project where f.project.id = :PROJECTID";
	...
	public List<FundsIncome> doJoinTransQueryFundsIncode(String id) {
		String sql = "select f from FundsIncome f left join fetch f.project where f.project.id = :ID order by f.changeTime desc";
		Map<String, Object> paramMap = new HashMap();
		paramMap.put("ID", id);
	...
	@Override
	public Attachment doJoinTransFindProjectTp(String id) {
		String sql = "select f from Attachment f where f.project.id = :PID";
