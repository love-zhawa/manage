...
import team.nercita.manage.cms.po.deptmanage.PlanLogYq;
...
public class WorkPlanServiceImpl extends BaseService implements WorkPlanService {
	...
	public Map<String, Object> doJoinTransFindWorkPlan(Integer goPage, Map<String, Object> paramMap) {
		...
		String sql;
		String countsql;
		if(user.getPost() == 0){//主任  查看所有计划
			sql = "select wp from WorkPlan wp left join fetch wp.user left join fetch wp.userGroup left join fetch wp.distUser left join fetch wp.logList where (:OWNID = :OWNID) ";
			countsql = "select count(wp) from WorkPlan wp where (:OWNID = :OWNID)  ";
		}else{
			sql = "select wp from WorkPlan wp left join fetch wp.user left join fetch wp.userGroup left join fetch wp.distUser left join fetch wp.logList where (wp.user.id = :OWNID or wp.distUser.id = :OWNID) ";
			countsql = "select count(wp) from WorkPlan wp where (wp.user.id = :OWNID or wp.distUser.id = :OWNID)  ";
		}
	...
	public WorkPlan doJoinTransFindWorkPlanById(String id) {
		String sql = "select w from WorkPlan w left join fetch w.user left join fetch w.distUser left join fetch w.userGroup left join fetch w.logList where w.id = :WORKPLANID";
	...
	public void doJoinTransUpdateWorkPlan(WorkPlan workPlan) {
		...
		oldWorkPlan.setUser(workPlan.getUser());//修改指给的人
		oldWorkPlan.setUserGroup(user.getUserGroup());//修改指给的人的小组
	...
	public void doJoinTransCheckWorkPlan(WorkPlan workPlan) {
		...
		if(workPlan.getEvaluate().equals(2)){
			oldWorkPlan.setEndTime(workPlan.getNewendTime());
			oldWorkPlan.setNewendTime(workPlan.getEndTime());
			oldWorkPlan.setYy(workPlan.getYy());
			
			PlanLogYq yq = new PlanLogYq();
			yq.setId(Generator.getUUID());
			yq.setWorkPlan(workPlan);
			yq.setYy(workPlan.getYy());
			yq.setNewEndTime(workPlan.getNewendTime());
			yq.setOldEndTime(workPlan.getEndTime());
			baseDao.save(yq);
		}
	...
	public List<WorkPlan> doJoinTransFindWorkPlanList(String userId, Date beginDate, Date endDate) {
		...
		String sql = "select w from WorkPlan w left join fetch w.user left join fetch w.userGroup left join fetch w.distUser where w.user.id = :USERID and (:BEGINDATE between w.beginTime and w.endTime or :ENDDATE between w.beginTime and w.endTime) order by w.createTime desc";
		Map<String, Object> queryMap = new HashMap<String, Object>();
		queryMap.put("USERID", userId);
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<WorkPlanVo> doJoinTransFindWorkPlans(String userId, int startIndex, int maxResult) {
		String sql = "select w from WorkPlan w where w.user.id = :USERID and (w.evaluate = 0) order by w.createTime desc";
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<PlanLogYq> doJoinTransQeuryYqById(String id) {
		String sql = "select p from PlanLogYq p where p.workPlan.id = :PLANID order by p.createTime desc";
		
		Map<String, Object> queryMap = new HashMap<String, Object>();
		queryMap.put("PLANID", id);
		
		return (List<PlanLogYq>) baseDao.findObjectList(sql, queryMap);
	}
}
