...
//		ProductStore productStore = baseDao.findObject(ProductStore.class, outStoreApply.getProductStore().getId());
	...
	public void doTransOutStore(String id, User user) {
		...
		outStoreApply.setOutTime(new Date());
		baseDao.update(outStoreApply);
	}

	@SuppressWarnings("unchecked")
	@Override
	public List<OutStoreApply> doTransFindAll() {
		String sql = "select a from OutStoreApply a left join fetch a.productType left join fetch a.project";
		return (List<OutStoreApply>)baseDao.findObjectList(sql, null);
	}

	@Override
	public List<IncomeLog> doTransFindIncomeLog(String id) {
  		String sql = "select p from IncomeLog p where p.cknumber = :ID";
		Map<String, Object> paramMap = new HashMap();
		paramMap.put("ID", id);
		
		List<IncomeLog> lists = new ArrayList<IncomeLog>();
		List<IncomeLog> list = (List<IncomeLog>) baseDao.findObjectList(sql, paramMap);
