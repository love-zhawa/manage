...
<form action="funds/other/list" method="post" id="expertsId">
  <div class="panel admin-panel">
    ...
    <table class="table table-hover text-center">
      ...
      <c:forEach items="${otherList}" var="other" varStatus="status">
			<tr>
	          ...
	          <td>${other.project.financialNumber }</td>
	          <td>${other.project.projectName }</td>
	          ...
	          <td>${other.remark }</td>
	          <td>
		      	<c:if test="${other.status == null }">未提交审核</c:if>
		      	<c:if test="${other.status == 0 }">已提交审核</c:if>
		      	<c:if test="${other.status == 1 }">审核通过</c:if>
		      	<c:if test="${other.status == 2 }">审核不通过</c:if>
		      </td>
	          <td>
		          <div  class="button-group">
		          <c:if test="${other.status == null || other.status == 2}">
		          	<shiro:hasPermission name="funds_other_edit">
			          	<a class="button border-main" onclick="edit('${other.id}')" href="javascript:;">
		          			<span class="icon-edit"></span> 编辑
		          		</a>
	          		</shiro:hasPermission>
	          		<shiro:hasPermission name="funds_other_sub">
		       			<a class="button border-green" onclick="subaudit('${other.id}')" href="javascript:;">
		          	...
		          	<c:if test="${other.status == 0 }">
			       		<shiro:hasPermission name="funds_experfee_audit">
			       			<a class="button border-dot" onclick="toaudit('${other.id}')" href="javascript:;">
				       			<span class="icon-rocket"></span> 审核
				       		</a>
			       		</shiro:hasPermission>
		       		</c:if>
		       		<c:if test="${other.status == 1 && (roleid == '2df370f1f420497bbd5a1b5ef8d3a6ee' || roleid=='96e79218965eb72c92a549dd5a330112')}">
		       			<shiro:hasPermission name="funds_other_edit">
				          	<a class="button border-main" onclick="edit('${other.id}')" href="javascript:;">
			          			<span class="icon-edit"></span> 编辑
			          		</a>
	...
	<script type="text/javascript">
		function todel(id) {
			if(confirm("确认删除？")){
				$.post("funds/other/del",{"id":id},function(data){
					if(data == 0) {
						alert("删除成功！");
					} else {
						alert("删除失败！");
	    ...
	    function subaudit(id){
			if(confirm("确认提交审核？")){
				window.location.href = "funds/other/sub/"+id;
			}
		}
		function toaudit(id){
			window.location.href = "funds/other/toaudit/"+id;
		}
	</script>
