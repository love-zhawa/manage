...
<div class="panel admin-panel">
  <div class="panel-head"><strong><span class="icon-pencil-square-o"></span>&nbsp;&nbsp;添加入库产品</strong></div>
  <div class="body-content">
    <form action="jxc/income/add" method="post" class="form-x" id="form-article-add" enctype="multipart/form-data">
          ...
          <select name="outStore.id" class="input" id="type" onchange="changemodel()">
				...
				<c:forEach items="${outList }" var="out">
				    <c:if test="${out.status eq 4 }">  
						<option value="${out.id}">${out.outNo }</option>
					</c:if>
				</c:forEach>
			</select>
        </div>
      ...
      <div class="form-group">
        ...
        <div class="field">
          <input type="text" class="input" id="manufacturer" name="manufacturer"/>
        </div>
      </div>
	...
	<script type="text/javascript">
		...
		function userSub(){
			...
			var amount = $("input[name ='amount']").val();//填写的入库的数量
			var amounts = $("#amounts").html();//出库的数量
			...
			if(eval(amount)>eval(amounts)){
				alert("入库数量不足！数量共:"+amounts);
			    return;
			}
			if(confirm("确认提交？")) {
