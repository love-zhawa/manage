    ...
    <table class="table table-hover text-center">
      ...
      <c:forEach items="${changeDeptList}" var="changeDept" varStatus="status">
			<tr>
	          ...
	          <td>
	          	<c:if test="${changeDept.changeType == 1 }">调岗</c:if>
	          	<c:if test="${changeDept.changeType == 2 }">借调（截止期限：<fmt:formatDate value="${changeDept.changeEndTime }" pattern="yyyy年MM月dd日"/>）</c:if>
	          	<c:if test="${changeDept.changeType == 3 }">调薪</c:if>
	          	<c:if test="${changeDept.changeType == 4 }">${changeDept.otherInfo}</c:if>
	          </td>
