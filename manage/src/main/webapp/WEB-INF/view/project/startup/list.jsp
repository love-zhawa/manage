  ...
  <div class="panel admin-panel">
    ...
    <div class="padding border-bottom">
      <ul class="search" style="padding-left:10px;">
        ...
        <li>搜索：</li>
        <li>
        	<select name="bussid" class="input" style="width:250px;">
	          	<option value="">请选择业务</option>
	          	<c:forEach var="buss" items="${bussList}">
	          		<option value="${buss.id }" <c:if test="${buss.id == bussid }">selected</c:if>>${buss.bussName }</option>
	          	</c:forEach>
	          </select>
        </li>
        <li>
        	<select name="projectid" class="input" style="width:250px;">
	          	<option value="">请选择横向课题</option>
        ...
        <li>
        	...
        	<select name="userid" class="input" style="width:250px;">
	          	...
	          	<c:forEach items="${lists}" var="user" varStatus="i">
	          		<option value="${lists[i.index][1]}" <c:if test="${lists[i.index][1] == userid }">selected</c:if>>${lists[i.index][1]}</option>
	          	</c:forEach>
	          </select>
        </li>
        <li>
