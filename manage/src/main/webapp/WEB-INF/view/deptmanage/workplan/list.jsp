...
<form action="workplan/list" method="post" id="searchForm">
  <div class="panel admin-panel">
    ...
    <div class="padding border-bottom">
      ...
      <shiro:hasPermission name="plan_add">
        ...
        <li>
          ...
          <input type="text" placeholder="请输入开始时间" id="startTime" name="startTime" autocomplete="off" class="input" style="width:200px; line-height:17px;display:inline-block"  value="${startTime}" />
          <input type="text" placeholder="请输入结束时间" id="endTime" name="endTime" autocomplete="off" class="input" style="width:200px; line-height:17px;display:inline-block"  value="${endTime}" />
      ...
      <c:forEach items="${workPlanList}" var="workPlan" varStatus="status">
			<tr>
	          ...
	          <td>
	          	...
	          	<c:if test="${workPlan.evaluate == 2 }"><a href="javascript:void(0)" onclick="showYq('${workPlan.id}')">已延期</a></c:if>
	          </td>
	          <td>
	...
	<!-- 显示延期详细情况 -->
	<div class='alertWindow1' id="alertWindow1" style="display:none;">
        <div class="bg_table bg_table1 hide-box" id="rgxq1">
        	<b class= 'closeBtn'  onclick='closeWindow1()'>×</b>
        	<h2 style="display:block;width:100%;text-align:center;font-weight:normal;margin-bottom:5px;">延期情况</h2>
        	<div class="content">	                    		
                <table class="table table-hover text-center">
                    <thead>
                        <tr>
	...
	<script type="text/javascript">
		...
		//显示延期详细情况
		function showYq(id){
			$.post("workplan/yq/"+id,function(data){
				$.each(data,function(index, item){
					var tr = "<tr>";
					tr += "<td>";
					tr += index+1
