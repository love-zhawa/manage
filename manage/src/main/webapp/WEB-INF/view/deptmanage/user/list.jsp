    ...
    <div class="padding border-bottom">
      <ul class="search" style="padding-left:10px;">
        ...
        <li>
         ...
         <input list="cars" placeholder="请输入搜索关键字" name="name" class="input" style="width:250px; line-height:17px;display:inline-block" autocomplete="off"/>
		<datalist id="cars">
          ...
          <select name="userGroupId" class="input" style="width:250px; line-height:15px;display:inline-block" >
				<c:forEach items="${userGroupList}" var="usergroup">
					<option value="${usergroup.id}"<c:if test="${usergroup.id eq userGroupId}">selected="selected"</c:if>>${usergroup.groupName}</option>
				</c:forEach>       	   		
       	   </select>
